...
export default function LogosHorizontalSection() {
      ...
      if (isInView && isScrollDisabled) {
        ...
        setVirtualScroll((prev) => {
          ...
          if (event.deltaY > 0) {
            ...
            newScroll = Math.min(
              prev + Math.abs(event.deltaY) * 0.8,
              maxScrollForEffect
            );
  ...
  useEffect(() => {
    ...
    if (backgroundRef.current) {
      const backgroundProgress = Math.max(
        progress * 3,
        virtualScroll > 0 ? Math.max(progress, 0.3) : 0
      );
      const finalProgress = Math.min(backgroundProgress, 1);

      gsap.to(backgroundRef.current, {
        backgroundColor: `rgb(${255 - (255 - 195) * finalProgress}, ${
          255 - (255 - 64) * finalProgress
        }, ${255 - (255 - 105) * finalProgress})`,
        duration: 0.6,
        ease: "power2.out",
      });
    }
    ...
    if (row1Ref.current) {
      ...
      const finalRow1Progress =
        virtualScroll > 0 ? Math.max(row1Progress, 0.2) : row1Progress;
    ...
    if (row2Ref.current) {
      ...
      const finalRow2Progress =
        virtualScroll > 50 ? Math.max(row2Progress, 0.15) : row2Progress;
    ...
    if (row3Ref.current) {
      ...
      const finalRow3Progress =
        virtualScroll > 100 ? Math.max(row3Progress, 0.1) : row3Progress;
  ...
  return (
    ...
    >
      ...
      {/* Content Container */}
      <div className="relative w-full px-[30px] lg:px-20 flex items-center justify-between h-screen">
        {/* Testo fisso a sinistra */}
        <div className="flex-shrink-0 z-10 max-w-md">
          <h2
            className={`text-4xl lg:text-5xl xl:text-6xl font-extralight leading-tight transition-colors duration-600 ${
          ...
          <p
            className={`text-lg font-light mt-6 transition-colors duration-600 ${
              virtualScroll >= 50 ? "text-white/80" : "text-gray-600"
            }`}
          >
            I migliori brand per offrire prodotti sicuri, certificati e di
            qualità.
          </p>
        </div>

        {/* 3 Righe di loghi sfalsate a destra */}
        <div className="flex-1 flex flex-col justify-center items-end space-y-8 mr-0 lg:mr-20 overflow-hidden">
          {/* Riga 1 */}
          <div
            ref={row1Ref}
          ...
          >
            {logoRows[0].map((logo, index) => (
              <div
                key={`row1-${index}`}
                className="p-4 w-[150px] h-[100px] lg:w-[220px] lg:h-[150px]"
              >
                <img
                  src={logo.src}
                  alt={logo.name}
          ...
          <div
            ref={row2Ref}
            className="flex gap-8 opacity-0 ml-8 lg:ml-16"
            style={{ transform: "translateX(-100%)" }}
          >
            {logoRows[1].map((logo, index) => (
              <div
                key={`row2-${index}`}
                className="p-4 w-[150px] h-[100px] lg:w-[220px] lg:h-[160px]"
              >
                <img
                  src={logo.src}
                  alt={logo.name}
          ...
          <div
            ref={row3Ref}
            className="flex gap-8 opacity-0 ml-16 lg:ml-32"
            style={{ transform: "translateX(-100%)" }}
          >
